...
def changeaccesskey(id):
    print(id)
    key = ''.join(random.choices(string.ascii_lowercase +string.digits, k=25))
    stmt = ibm_db.exec_immediate(conn,"UPDATE adminusers SET random_key ='"+ key +"' WHERE user_id='"+ str(id) +"';")
    if ibm_db.exec_immediate(conn, stmt):
        return "Done"
    return "Error"
...
def savenewstodbobj(newsid,userid):
    stmt = "insert into saved (uid,newsid) values ('"+str(userid)+"','"+ str(newsid)+"' );"
    if ibm_db.exec_immediate(conn, stmt):
        return "Done"
    return "Error"
...
def getsaveditemsfromdb(id):
    d1 = today.strftime("%d/%m/%Y")
    stmt = ibm_db.exec_immediate(conn, "select newsid from saved where uid='"+str(id)+"';")
    returndictionary = {}
    while ibm_db.fetch_row(stmt) != False:
        newsid =  ibm_db.result(stmt, 0)
        print(newsid)
        stmt1 = ibm_db.exec_immediate(conn, "select hid,headlinesdata from headlines where hid='"+str(newsid)+"';")
        while ibm_db.fetch_row(stmt1) != False:
            value = ibm_db.result(stmt1, 1)
            print(value)
            returndictionary.update({newsid: str(value).replace("U-001",",").replace("U-002",'"').replace("U-003","'")})
            print("Strored in return dictionary")
    print("\n\n\nReturn")        
